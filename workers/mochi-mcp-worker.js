...
async function handleMcp(request, env) {
  ...
  if (methodName.startsWith("notifications/")) {
    if (methodName === "notifications/log" && params?.level) {
      console[params.level === "error" ? "error" : params.level === "warn" ? "warn" : "log"](
        "MCP notification",
        params?.message ?? ""
      );
    }
  ...
  try {
    if (methodName === "initialize") {
      const requestedProtocol = params?.protocolVersion || "2025-06-18";
    ...
    if (methodName === "ping") {
      return Response.json(ok(id, { pong: true }), { headers: corsHeaders() });
    }
    ...
    if (methodName === "tools/call") {
      ...
      const outputs = toMcpOutputs(data);
      const mergedContent = [];
      for (const output of outputs) {
        if (Array.isArray(output?.content)) {
          mergedContent.push(...output.content);
        }
      }
      return Response.json(ok(id, {
        outputs,
    ...
    return Response.json(jerr(id, -32601, `Method not found: ${methodName}`), { status: 404, headers: corsHeaders() });
  } catch (e) {
    const status = e?.status ?? 500;
    const outputs = toMcpOutputs({ error: e?.message ?? String(e) });
    const mergedContent = [];
    for (const output of outputs) {
      if (Array.isArray(output?.content)) {
        mergedContent.push(...output.content);
